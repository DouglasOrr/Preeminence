...
feel a bit cumbersome. You'll probably find that the big challenge is getting a `PlayerState` instance
...
event = game.next_event(method='act')
state = event.state

state.world.owners = [0, 1, 0, 0, 0, 0]
state.world.armies = [2, 2, 2, 2, 10, 2]
IPython.display.display(event.state.world)  # Requires: import IPython.display

# try implementing your agent's act() method using "state"
```

_WARNING - if you copy your code from a cell into your agent class, be very careful to ensure all the
variables come from the method args (it is very easy to use the wrong state/armies/owners!)_
...
class Game:
    ...
    event = game.next_event(method='reinforce', player_index=0)   # find the next matching action
    ```
    """
